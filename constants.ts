...
export const EXPENSE_RATIOS: Record<string, number> = {
  ...
  'SPY': 0.0009, // 0.09%
  'QQQM': 0.0015, // 0.15%
  'SPYM': 0.0003, // Assuming similar to low cost SPY equivalents
  'IVV': 0.0003
};
...
export const AVAILABLE_TICKERS = ['VOO', 'QQQ', 'SPY', 'QQQM', 'SPYM'];
...
export const PORTFOLIO_PRESETS: PortfolioPreset[] = [
  {
    label: 'conservative',
    risk: 'conservative',
    items: [{ ticker: 'VOO', percentage: 100 }]
  },
  {
    label: 'balanced',
    risk: 'balanced',
    items: [{ ticker: 'VOO', percentage: 60 }, { ticker: 'QQQ', percentage: 40 }]
  },
  {
    label: 'aggressive',
    risk: 'aggressive',
    items: [{ ticker: 'QQQ', percentage: 100 }]
  }
];
...
export const TRANSLATIONS: Record<Language, any> = {
  en: {
    ...
    tipsList: [
      ...
      "The 'Expected Return' auto-updates based on historical averages.",
      "Expense ratios are automatically deducted from growth."
    ],
    ...
    expenseRatio: "Avg Expense Ratio",
  ...
  ja: {
    ...
    tipsList: [
      ...
      "結果は、ETF経費率（信託報酬）を差し引いて計算されます。",
      "PDF出力は、結果のみを含むレポート形式になります。"
    ],
